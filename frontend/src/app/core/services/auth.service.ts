    ...
    try {
      ...
      try {
        ...
        if (!authHeader || !authHeader.startsWith('Bearer ')) {
          console.error('[handleCallback] Authorizationヘッダーなし');
          return false;
        }
        ...
        console.log('[handleCallback] 業務JWT保存完了 → true返却');
        ...
        if (httpError instanceof HttpErrorResponse && httpError.status === 401) {
          const errorCode = this.extractErrorCode(httpError);
          console.warn('[handleCallback] /auth/verify が401:', errorCode);
          ...
          // ※ login() → window.location.hrefでページ全体リダイレクト
          // ※ リダイレクト後、Entra IDのコールバックで
          //   CallbackComponent.ngOnInit() → handleCallback()が再度実行される
          this.login();
          
          // このJavaScript実行はここで終了（return false）
          // 次のhandleCallback()は、リダイレクト後のコールバックで呼び出される
          return false;
        }
